...
import os
from .repvgg import create_RepVGG_D2se


class RepVGG_D2se(nn.Module):
    def __init__(self, class_num=248):
        super(RepVGG_D2se, self).__init__()
        pr_model = create_RepVGG_D2se(deploy=False)
        pr_model.load_state_dict(torch.load('./model/RepVGG/RepVGG-D2se-train.pth'))
    ...
    def save(self, circle):
        name = "./weights/RepVGG_D2se" + str(circle) + ".pth"
...
if __name__ == '__main__':
    ...
    model = create_RepVGG_D2se(deploy=False)
    model.load_state_dict(torch.load('./RepVGG/RepVGG-D2se-train.pth'))
    print(model)
