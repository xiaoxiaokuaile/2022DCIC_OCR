...
class Cutout(object):
    ...
    Args:
        n_holes (int): Number of patches to cut out of each image. 每个图像要剪切的切片数目
        length (int): The length (in pixels) of each square patch. 每个方形片面的长度
    """
    ...
    def __call__(self, img):
        img = transforms.ToTensor()(img)  # 将numpy图片转化为tensor
        ...
        for n in range(self.n_holes):
            ...
            y1 = np.clip(y - int(self.length * h) // 2, 0, h)
            y2 = np.clip(y + int(self.length * h) // 2, 0, h)
            x1 = np.clip(x - int(self.length * w) // 2, 0, w)
            x2 = np.clip(x + int(self.length * w) // 2, 0, w)
...
def warp(img):
    ...
    prob_distort = 0    # 扭曲的概率
    prob_reversal = 0     # 反转图像
    # --------------------------- tia扰动 ---------------------------------
    ...
    if random.random() <= prob_reversal:
        new_img = 255 - new_img
